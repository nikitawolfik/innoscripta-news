...
Three things about this codebase are deliberate and would otherwise look like
mistakes.
...
Each adapter converts through the same `src/server/node-adapter.ts`, because
two of the three hand the handler Node's own request and response rather than
the Web pair it takes. `src/server/` and `api/` are also the one place that
does not use the `~/` alias: those files get compiled by toolchains that know
nothing of the project's path mapping, and resolving without one is what keeps
a single handler buildable by all three.
